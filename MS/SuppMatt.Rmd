...
Similarity in distribution or diet may have a nonlinear relationship with phylogenetic distance.  This could be caused by both biological and statistical effects.  Biological, because the fixed compositon of the prey community limits how different two predators can become. Statistical, because Pianka's similarity index is bounded by 0 and 1 and therefore an OLS linear model could potentially predict impossible (ie $< 0$ or $> 1$) values. We fit several nonlinear equations to our data in order to quantify how the amount of divergence in distribution or diet between predators changed over evolutionary time.
...
We divided the experimental design above into three separate hypothesis tests.  First, we examine the effect of predator species number by comparing the five control replicates with the means of the four single species treatments and the means of the three two-species treatments.  This tests whether species number *per se* has an effect on our response variables.  Secondly, we compared the four different predator taxa, to see if any of these differed in their effect on the community.  Finally, we contrast our predator diversity treatments to see how increasing PD of the predator assemblage affects our response variables. Because the first and third hypotheses suggest a ranking of the independent variables (species number and phylogenetic diversity) we used ordinal contrasts in the corresponding linear models.
...
**Table S7**: The effect of predator species number on all 5 response variables.  We used ordinal contrasts to investigate how an increasing number of predator species influenced the response.  Treatment order was none < one < two species. (See Table S6).
```{r results='asis'}
...
**Table S9** : predator species effects.  These linear models use treatment contrasts, which contrast all treatments with *L. andromache*.  Coefficents (standard error) are given for *L. andromache*, and differences between coefficients (standard error of difference) for all other predators.
...
**Figure S3**: The effect of predator species identiy on each of our response variables.  Each dot represents the observed value for a different predator treatment.  Green dots represent group means.  
...
twopred_lm <- twopred %>% 
  gather(resp,val,-treatment) %>%
  group_by(resp) %>%
  mutate(treatment = treatment %>% as.character %>%
...
**Table S11**: Tukey post-hoc tests for differences between predator phylogenetic diversity treatments on all response variables.
