...
dist_sim <- read.csv(file = "analyses/distributional_similarity.csv", 
                     stringsAsFactors = FALSE)

diet_sim <- read.csv(file = "analyses/diet_similarity.csv",
                     stringsAsFactors = FALSE)

prop.eaten <- read.csv(file = "analyses/proportion.eaten.csv")
...
cortable <- c("total.surv","fine","decomp","growth","N") %>%
  ...
  group_by(var1,var2) %>%
  do(cor.test(pd[[.$var1]],pd[[.$var2]])[1:4] %>% as.data.frame) %>%
