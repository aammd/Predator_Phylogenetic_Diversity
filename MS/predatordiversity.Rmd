...
**Figure 2**:  Adding predators reduces insect emergence, probably because they eat the prey.
 However, increasing predator number raises survival again. This is because the predators are interfering with each other.

```{r}
pred_number <- pred_present %>%
  filter(ispred != "absent") %>%
  mutate(npred = ifelse(treatment %in% c("andro","tabanid","elong","leech"), "one","two") )
