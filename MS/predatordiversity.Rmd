...
models_to_fit <- data.frame(formulae = c(
  "overlap ~ a * (phylodistance)^2 + b * phylodistance + c",
  "overlap ~ peak * exp(-1 * (phylodistance)^2 / a)",
  "overlap ~ b * exp(a * phylodistance)",
  "overlap ~ c * exp(a * phylodistance) / (c * exp(a * phylodistance) + (1 - c))")
  ) %>% tbl_df()
...
metabolic_occur_phylo  %>%
  ...
  mutate(pred_m2 = predict(models$bellshaped[[1]],newdata = list(phylodistance = phylodistance)),
         ) %>% 
