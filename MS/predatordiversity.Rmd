...
#   summarise(mean_type = mean(meanval))
cntrl_val <- pd %>% 
  select(treatment,total.surv,fine,decomp,growth,N) %>%   
  gather(response,value,-treatment) %>%
...
models %>%
  ...
  stargazer(.,type = "html",column.labels = models$response %>% as.character)

# lapply(models$mod,TukeyHSD)

#models$mod %>% lapply(.,stargazer,type = "html")

# cbind(grepl(".+ \\+ .+",pd$treatment),pd$treatment)
```
