...
diet_predictions <- x_sequence %>% tbl_df %>%
  mutate(pred_m2 = predict(observed_fit,
         ...
         category = factor("diet",levels = c("diet", "distribution"))
         )
```
...
fig1 <- rbind(distribution, diet) %>%
  ...
  geom_line(aes(x = phylodistance, y = pred_m2, size = 1),data = diet_predictions) +
  geom_line(aes(x = phylodistance, y = upper, size = 1),data = diet_predictions,linetype = "dashed") +
  geom_line(aes(x = phylodistance, y = lower size = 1),data = diet_predictions,linetype = "dashed")
