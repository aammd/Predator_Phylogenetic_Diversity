...
foodweb %.%
  dcast(formula=Prey.species~predator.names,
        value.var="eaten.numeric",fun.aggregate=length) %.%
  data.frame(row.names="Prey.species") %.%
  select(Hirudinidae:Tabanidae.spC) %.%
  as.matrix()
...
diet_distance <- foodweb %.%
  dcast(formula=Prey.species~predator.names,value.var="eaten.numeric",fun.aggregate=sum) %.%
  data.frame(row.names="Prey.species") %.%
  select(Hirudinidae:Tabanidae.spC) %.%
  as.matrix() %.%
  t() %.%
