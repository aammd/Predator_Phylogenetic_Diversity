...
models <- diet_overlap_phylo %>% 
  ...
  do(quadratic = nls(overlap ~ a * (phylodistance)^2 + b * phylodistance + c, 
     ...
     linear = lm(overlap ~ phylodistance, data = .,weights = nspp),
...
fig1 <- diet_overlap_phylo %>% 
  ungroup %>%
  select(phylodistance,overlap,nspp) %>%
  ggplot(aes(x = phylodistance, y = overlap,size = nspp)) + 
  geom_point(shape  = 21) + 
  geom_point(colour = "black", fill = "darkgrey", shape  = 21, alpha = 0.6) + 
...
predictions  <-  replicate(
  100,{
    ...
    model = failwith(NULL,nls)(overlap ~ b * exp(a * phylodistance), 
                               data = boot, 
                               start = list(a = -0.0004, b = 1),
                               weights = nspp)
    # Output predictions at each point that we'll want to plot later
    if(!is.null(model)) predict(model, x_sequence) else rep(NA,length(x_sequence))
    }
  )

#  browser()nls(overlap ~ b * exp(a * phylodistance),
                       data = .,
                       start = list(a = -0.0004, b = 1),
                       weights = nspp)
