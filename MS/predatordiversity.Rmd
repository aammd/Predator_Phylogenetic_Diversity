...
# pn <- unique(feeding_trials$predator.names) 
# #names(pn) <- unique(feeding_trials$predator.names)
# prednames <- outer(pn,pn,FUN=paste,sep="_") 
# 
# predpairs <- prednames %.%
...
diet_overlap_phylo %.%
  filter(nspp>1) %.%
  ggplot(aes(x=phylodistance,y=overlap,size=nspp))+geom_point()+stat_smooth(method="lm")
