...
fig1 <- metabolic_occur_phylo %>% 
  ungroup %>% 
  mutate(PDgrp = phylodistance %>%  cut(breaks = 5)) %>%
  select(PDgrp,phylodistance,overlap) %>%
  arrange(phylodistance) %>% 
  group_by(PDgrp) %>%
  ...
  geom_point(colour = "black", fill = "darkgrey", size = 4, shape  = 23) + 
...
models <- metabolic_occur_phylo %>% 
  ungroup %>% 
  select(phylodistance,overlap) %>%
  arrange(phylodistance) %>%
  (function(x) {
    m1 <- nls(overlap ~ a * (phylodistance)^2 + b * phylodistance + c, 
              data = x, 
              start = list(a = -0.0000008, b = 0.0008, c = 1))
    m2 <- nls(overlap ~ peak * exp(-1 * (phylodistance)^2 / a), 
              data = x, 
              start = list(a = 900000, peak = 1))
    m3 <- nls(overlap ~ b * exp(a * phylodistance),
              data = x,
              start = list(a = -0.0004, b = 1))
    m4 <- nls(overlap ~ c * exp(a * phylodistance) / 
                (c * exp(a * phylodistance) + (1 - c)),
              data = x,
              start = list(a = -0.007, c = 0.9))
    m5 <- lm(overlap ~ phylodistance, data = x)
    m6 <- nls(overlap ~ z,
              data = x,
              start = list(z = 0.4))
...
metabolic_occur_phylo  %>%
  ...
  mutate(pred_m2 = predict(models$m2,newdata = list(phylodistance = phylodistance)),
         ) %>% 
  gather(model,prediction,-phylodistance) %>%  
  (function(x) fig1 + geom_line(aes(x = phylodistance, y = prediction),data = x)) 

# AIC(models$m1,models$m2,models$m3,models$m4,models$m5,models$m6) %>% 
#   (function(df){
#   data.frame(df,rnames = df %>% row.names)}) %>%
#   arrange(AIC)
...
foo <- data.frame(one = c("a","b"), two = c("C","D"), row.names = 1)

mutate(foo, test = row.names(foo))

```


```{r FIG_feeding_trial_as_phylo, fig.cap="Figure 2:  Diet overlap decreases with phylogenetic distance, based on 237 feeding trials with the 8 major predator taxa found in this system.  Diet similarity is calculated as Pianka's index of niche overlap. The best-fit regression, weighted by sample size, is shown with the 95% CI.",dev='png'}
...
diet_overlap_phylo %>%
  ...
  +xlab("phylogenetic distance") + ylab("Overlap in diet breadth") + mytheme
...
ggplot(subset(summarize_randoms_phylo,summarize_randoms_phylo$variable=="survival"),
       aes(x=phylodistance,y=mean))+geom_errorbar(aes(ymin=lower, ymax=upper),width=0)+geom_point(size=3)+ylab("Mean treatment difference, Control-Treatment")+xlab("Time (Mya)") + mytheme
# 
