...
pianka <- function(df){
  mat <- as.matrix(df[,-1])
  squares <- mat^2
  prod_sq <- apply(squares,2,prod)
  sum_prod_sq <- sum(prod_sq)
  prod <- apply(mat,2,prod)
  sum_prod <- sum(prod)
  overlap <- sum_prod/sqrt(sum_prod_sq)
  nspp <- ncol(mat)
...
dietoverlap_lm <- with(diet_overlap_phylo,lm(overlap~phylodistance,weights=nspp))
#summary(dietoverlap_lm)

#ggplot(diet_overlap_phylo,aes(x=phylodistance,y=overlap,size=nspp))+geom_point()
