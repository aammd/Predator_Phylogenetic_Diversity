...
(`r round(mean(pd_no_out$mass.g.), 2)`g ± `r round(sd(pd_no_out$mass.g.), 2)`). 
...
We did not find any significant relationship between habitat distribution (measured as Pianka's index of niche overlap) among predator species and the phylogenetic distance between them (Figure 1a, `r F_text(meta_phylo_lm_summary)`). Indeed we often found multiple predator species co-occurring in the same bromeliads (mean `r nspp$meanpred` ± `r nspp$sdpred` predator species per plant).  This indicates that all `r metabolic$Taxa %>% unique %>% length` predator species have roughly similar habitat distributions at the level of the bromeliad (among `r ncol(metabolic)-1` bromeliads studied). We were able to sample a wide range of phylogenetic relatedness, including two groups of 
