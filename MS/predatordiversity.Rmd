...
We tested for effects of predator species on community processes, both alone and in combination, with a manipulative experiment where identical communities were exposed to treatments of either a single predator, or a range of phylogenetically-diverse pairs of predatorss. We divided the analysis of this experiment into three parts. First, we used Pianka's index (as above) to measure the similarity between surviving prey communities among the four treatments with single predator species, and related this similarity to phylogenetic distance. If predator feeding choices are phylogenetically conserved, then we expect a negative relationship, with close relatives leaving behind similar communities of prey. 

Second, we examined how predators affected the five community and ecosystem responses we described above. For each response, we combined our treatment levels into four different contrasts, each of which examine a different aspect of predator impact. First, we contrasted the control treatment (predators absent) with the mean responses of all seven treatments that did contain predators. Second, we contrasted the means of all monocultures with the means of all pair treatments. Third, we contrasted all four predator species and finally we contrasted all three pairs of predators. We analyzed each of these responses with one-way ANOVA. 
...
We did not find any significant relationship between niche overlap among pairs of predator species and the phylogenetic distance between those predator species (`r F_text(meta_phylo_lm_summary)`). Indeed we often found predator species co-occurring in the same bromeliads (`r nspp$meanpred` ± `r nspp$sdpred` predator species per plant).  This indicates that predator species all have roughly similar habitat distributions at the level of the bromeliad. This observational data contains more species than our other datasets: in `r ncol(metabolic)-1` bromeliads, we found 
