...
modlist <- list(
  ...
  number = pd2 %>% 
    ...
    group_by(npred, treatment, response) %>%
    summarise(mean = mean_na(resp_val)),
  phylodist = pd2 %>%
    filter(npred == "two")
  )
...
Response | Predator Presence | Identity | Richness
-------- | ------------------| ---------|---------
Total prey survival  |   `r prF("total.surv", "presence")` |  | 
FPOM (g)     | `r prF("fine", "presence")` |  | 
Decomposition (g)  | `r prF("decomp", "presence")` | |
Bromeliad growth  | `r prF("growth", "presence")` | |
