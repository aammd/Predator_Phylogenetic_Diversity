...
[@Werner1984].  Such ontogenetic changes in diet breadth can radically influence
...
level kelp food web [@Byrnes2006]. The complex traits that underlie such species
interactions ("higher-order" interactions, Sih *et al.* [-@Sih1998a]) may also
correlate with phylogeny; if so, then phylogenetic diversity will be a better
predictor of ecosystem function than richness alone [@Cadotte2009]. For example,
...
trait-mediated indirect effects (TMII, [@Werner2003]) can be difficult to
predict; however phylogenetic relatedness has been suggested as a possible means
of predicting the combined effect of predators [@Naisbit2011]
...
hypotheses concerning similarity in distribution, diet and top-down ecosystem
effects, using a natural mesocosm: the community of invertebrates living within
bromeliads. Bromeliads (Bromeliaceae) are epiphytic plants native to the
Neotropics. Many bromeliad species contain water, detritus and a complex insect
food web within their leaves; the decomposition of this detritus supplies
nutrients for the bromeliad [@Benzing2000]. The small size of these habitats
permits direct manipulations of entire food webs, manipulations which would be
difficult in most natural systems.  Within this aquatic food web, damselfly
larvae (e.g. *Leptagrion* spp., Odonata:Coenagrionidae) are important predators;
their presence dramatically alters community dynamics (e.g. decreases insect
...
3. *ecosystem-level effects*: Our experiments at the level of the whole habitat patch (i.e. a single bromeliad) allow us to test hypotheses about direct and indirect effects of predator combinations:
    (a) Closely-related predators will have similar effects on the community. This will occur if related predators share similar trophic interactions (e.g. predation rate, diet breadth). Our monoculture treatments allow us to assess the effect of each predator on both a complete prey community and ecosystem function.  
...
tract the effects on detrital decomposition on bromeliad nutrition, we enriched
...
species assayed with both predator taxa.  In the survey data preference ($p_{ik}$) is defined as the proportion of a predator's totaly metabolic capacity found in a particular bromeliad, in diet trials it is the
...
write.csv(occur_aic,file = "./analyses/distributional_similarity.csv")
...
diet_predictions <- x_sequence %>% tbl_df %>%
  mutate(pred_m2 = predict(observed_fit,
         ...
         category = "diet",
         category = factor(category,levels = c("diet", "distribution"))
         )
```
...
exper <- pd %>%
  ...
  ungroup %>%
  select(phylodistance,overlap,nspp) %>%
  mutate(category = "experiment")

rbind(distribution, diet, exper) %>%
  mutate(category = factor(category, levels = c("distribution", "diet", "experiment"))) %>%
  ...
  geom_line(aes(x = phylodistance, y = pred_m2), size = 0.5, data = diet_predictions) +
  geom_line(aes(x = phylodistance, y = upper),size = 0.5, data = diet_predictions,linetype = "dashed") +
  geom_line(aes(x = phylodistance, y = lower),size = 0.5, data = diet_predictions,linetype = "dashed") + 
  mytheme
