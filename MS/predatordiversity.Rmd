...
```{r FIG_experiment_responses,fig.cap="Figure 3: Treatment responses for the manipulative experiment.  Points are means, and horizontal lines represent minimum and maximum values for each response.  n=5 for all means.",dev='svg'}
# pd_long <- pd %.%
...
#   transform(treatment=factor(treatment,levels=c("control","andro","elong","leech","tabanid","elong + andro","elong + leech","elong + tab"))) %.%
#   arrange(treatment) %.%
#   mutate(treatment=ifelse(treatment=="elong","_L. elongatum_",treatment),
#          treatment=ifelse(treatment=="andro","_L. andromache_",treatment),
#          treatment=ifelse(treatment=="elong + andro","_L. elongatum_ + _L. andromache_",treatment),
#          treatment=ifelse(treatment=="elong + leech","_L. elongatum_ + leech",treatment),
#          treatment=ifelse(treatment=="elong + tab","_L. elongatum_ + tabanid",treatment)     
#          )

  
#   rename(c("total.surv"="surviving insects",
#            "fine"="production of fine detritus",
#            "decomp"="decomposition of coarse detritus (g)",
#            "growth"="bromeliad growth (cm)"))

# kable(exp_summary[c(2,1,3,7,8,4,5,6),],row.names=FALSE,format="pandoc")
# 
# xtable(exp_summary[c(2,1,3,7,8,4,5,6),],) %.%
#   print(type="html")
...
#print(xtable(test),type="html")

#kable(test)

#%.%
#pander(test,style="grid",split.tables="Inf",justify=c("left",rep("centre",5)))
...
# grid.arrange(foo,heights=17)
# 
# surv <- glm(data=pd,total.surv~leech+andro+tab+elong,family="poisson")
# fine <- lm(data=pd,fine~leech+andro+tab+elong)
# 
# stargazer(surv,fine, title="Regression Results",
# dep.var.labels=c("survival","fine"),covariate.labels=,single.row=TRUE,type="text")
```
