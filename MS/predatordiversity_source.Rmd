...
All predators showed a very generalist diet breadth, consuming nearly 100% of all prey offered to them.  However, more phylogenetically distant predators preferred slightly different diets, as measured by euclidian distance between feeding trial outcomes (F~`r diet_phylo_lm_summary $fstatistic[["numdf"]]`,`r diet_phylo_lm_summary $fstatistic[["dendf"]]`~=`r diet_phylo_lm_summary $fstatistic[["value"]]`,P=`r pf(diet_phylo_lm_summary $fstatistic[1],diet_phylo_lm_summary $fstatistic[2],diet_phylo_lm_summary $fstatistic[3],lower.tail=FALSE)`)  Regression was weighted by the number of trials conducted.

### Ecosystem-level effects and phylogenetic distance

```{r experimentsummary}
(mean(pd$total.surv[pd$treatment!="control"])-mean(pd$total.surv[pd$treatment=="control"]))/sd(pd$total.surv)

preddiff <- (mean(pd$total.surv[pd$treatment!="control"])-mean(pd$total.surv[pd$treatment=="control"]))/mean(pd$total.surv[pd$treatment=="control"])
...
Predators had a large effect on prey survivorship -- all predator treatments combined reduced prey survivorship by `r abs(predeffect())`% relative to the predator-free control.  Nitrogen transport to bromeliad leaves was slightly decreased relative to controls (`r predeffect("N")`%), and was only higher than the control in treatments including Tabanid predators.  We found a similar pattern for plant growth: on average, predators had a `r predeffect("growth")`% effect on growth of bromeliad leaves (mm), though Tabanids seemed to create a slight increase. The decomposition of coarse detritus and production of fine organic matter showed no obvious pattern related to the mere presence of predators.


<!--
How do individual predator treatments differ from each other?

How do monocultures and polycultures differ?

Finally, most importantly: how to non-additive effects change with phylogenetic diversity?
-->
