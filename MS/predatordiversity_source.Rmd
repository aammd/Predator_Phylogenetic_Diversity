...
## to merge with the phylogenetic distance
allpred.distance_df <- melt(allpred.distance.matrix)[melt(upper.tri(allpred.distance.matrix))$value,]
names(allpred.distance_df) <- c("phylopred1", "phylopred2", "phylodistance")
allpred.distance_df$sp.pair <- paste(allpred.distance_df$phylopred1,
                                     allpred.distance_df$phylopred2,sep="_")

## note that the nomeclature of the columns keeps `sp.pair` as the only shared name among columns.

## merge them
dataset <- merge(allpred.distance_df,metabolic_cor_df)
ggplot(dataset,aes(x=phylodistance,metadistance))+geom_point()+stat_smooth(method="lm", se=FALSE)
#### diet similarity ####
diet_df <- melt(dist.mat)[melt(upper.tri(dist.mat))$value,]
names(diet_df) <- c("dietpred1", "dietpred2", "dietdistance")
diet_df$sp.pair <- paste(diet_df$dietpred1,diet_df$dietpred2,sep="_")

## merge(diet_df,allpred.distance_df)
