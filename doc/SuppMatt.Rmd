...
timetree_table_links <- timetree_table %>% 
  mutate(Referencelink = paste0(Reference, Year)
         # Referencelink = paste0("\\hyperref[", Referencelink, "]{''", urls, "''}")
         ) %>% 
  select(-Reference, -Year) %>% 
  rename(Reference = Referencelink)
```
